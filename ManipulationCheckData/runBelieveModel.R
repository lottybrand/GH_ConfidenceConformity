...
# Read the data
tempData = read.delim('believeTask.txt')
...
# Dependent variable
Believe <- tempData$BelieveTask
NObservations <- length(Believe)
...
jags.data = list('ParticipantID','Sex','Condition','NObservations','Believe')
...
my.samplesBelieve <- as.mcmc(my.model)
save('my.samplesBelieve',file =  'my.samplesBelieve.dump')

# Lets focus on the relevant parameters (ignore participant and question pars)
relPars = c('b_sex','b_cond','b_sex_cond')

# View the parameter densities and trace
plot(my.samplesBelieve[,relPars])

# View summary stats
my.summary = summary(my.samplesBelieve[,relPars])
my.summary
...
max(as.matrix(my.samplesBelieve[,'logLike']))